...
help:
	...
	@echo "  make test PATTERN=easing                 # Match test/**/*easing*_spec.lua"
	@echo "  make test PATTERN=notify                 # Match test/**/*notify*_spec.lua"
	@echo "  make test PATTERN=test/notify_spec.lua   # Full path"
...
# Examples:
#   make test PATTERN=test/notify_spec.lua
#   make test PATTERN=easing  (shorthand for test/**/*easing*_spec.lua)
test: install-deps
...
clean:
	...
	@rm -rf test/*.out
	@rm -rf *.swp
	@rm -rf /tmp/notify_nvim_test_* 2>/dev/null || true
